...
def shard_dis(images, captions, model, text_lens=None, shard_size=128, mode='i2t'):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            if mode == "i2t":
                ...
                im = Variable(torch.from_numpy(images[im_start:im_end]), volatile=True).float().cuda()
                a = Variable(torch.from_numpy(captions[cap_start:cap_end]), volatile=True).float().cuda()
                sim = model.i2i(im, a)
            elif mode == "i2a":
                im = Variable(torch.from_numpy(images[im_start:im_end]), volatile=True).float().cuda()
                a = Variable(torch.from_numpy(captions[cap_start:cap_end]), volatile=True).float().cuda()
...
def norm_loss(pred, eps=1e-8):
    pred = torch.pow(pred, 2).sum(dim=-1, keepdim=True).clamp(min=eps).sqrt()
    label = torch.ones_like(pred).cuda()
    loss = (pred - label).mean()
    return torch.abs(loss)

from loss_func import MMD
def intra_loss(feature, feature_t):
    mmd_loss = MMD.mmd_rbf(feature, feature_t)
    return torch.min(mmd_loss, torch.tensor(200.0))
...
def multi_source_triplet_loss(scores, margin=0.2, max_violation=False):
    score_vt, score_va = scores
    size = score_vt.shape[0]

    loss_vt = calcul_loss(score_vt, size, margin, max_violation=max_violation)
    loss_va = calcul_loss(score_va, size, margin, max_violation=max_violation)

    return 0.5 * (loss_vt + loss_va)


if __name__ == "__main__":
    from torch.autograd import Variable
    input_vec = torch.tensor(np.random.random((10,10,10)))

    # implement
    loss = multi_source_triplet_loss(input_vec)
    print(loss)
