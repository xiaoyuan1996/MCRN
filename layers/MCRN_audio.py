...
class pre_model(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(512*8*8,38)
...
class MCRN_audio(nn.Module):
    def __init__(self, opt):
        super(MCRN_audio, self).__init__()
        self.resnet = ResNet(BasicBlock, [2,2,2,2])

        self.linear = nn.Linear(512, opt['embed']['embed_dim'])
    ...
    def forward(self, x):
        ...
        out = torch.mean(out.view(out.shape[0], 512, -1), dim=-1)
...
if __name__ == "__main__":
    ...
    model = MCRN_audio(options['model'])
